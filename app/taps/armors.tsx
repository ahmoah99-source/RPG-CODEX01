        ...
        <View style={styles.bonusGrid}>
          {item.body_bonus > 0 && (
            <View style={styles.bonusItem}>
              <Text style={styles.bonusLabel}>جسد +{item.body_bonus}</Text>
            </View>
          )}
          {item.spirit_bonus > 0 && (
            <View style={styles.bonusItem}>
              <Text style={styles.bonusLabel}>روح +{item.spirit_bonus}</Text>
            </View>
          )}
        </View>
        <View style={styles.defenseRow}>
          <View style={styles.defenseBadge}>
            <Text style={styles.defenseText}>دفاع x{item.defense_multiplier}</Text>
          </View>
        </View>
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.addButton} onPress={openAddModal}>
        ...
        <Text style={styles.addButtonText}>إضافة درع جديد</Text>
      </TouchableOpacity>
      {armors.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Shield size={64} color="#475569" />
          <Text style={styles.emptyText}>لا توجد دروع</Text>
        </View>
      ) : (
        <FlatList
          data={armors}
